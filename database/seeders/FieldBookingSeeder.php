    ...
    public function run(): void
    {
        ...
        if (! empty($sqliteInfo) && isset($sqliteInfo[0]->sql)) {
            ...
            if (preg_match('/CHECK\s*\(\s*["\']?status["\']?\s+IN\s*\((.*?)\)\s*\)/i', $tableSql, $matches)) {
                ...
                foreach (['paid', 'completed', 'success', 'approved', 'Success'] as $v) {
                    if (in_array($v, $allowedArr)) {
                        $successStatus = $v;
                        break;
                    }
                }
                foreach (['pending', 'unpaid', 'processing', 'Pending'] as $v) {
                    if (in_array($v, $allowedArr)) {
                        $pendingStatus = $v;
                        break;
                    }
                }
                foreach (['refunded', 'failed', 'cancelled', 'Refunded'] as $v) {
                    if (in_array($v, $allowedArr)) {
                        $refundedStatus = $v;
                        break;
                    }
        ...
        foreach ($fields as $index => $fieldData) {
            ...
            $bookingData = [];
            if (in_array('field_id', $bookingColumns)) {
                $bookingData['field_id'] = $field->id;
            }
            ...
            if (in_array('total_amount', $bookingColumns)) {
                $bookingData['total_amount'] = $field->price_per_hour * 2;
            }
            if (in_array('total_price', $bookingColumns)) {
                $bookingData['total_price'] = $field->price_per_hour * 2;
            }
            if (in_array('price', $bookingColumns)) {
                $bookingData['price'] = $field->price_per_hour * 2;
            }
            if (in_array('status', $bookingColumns)) {
            ...
            if (in_array('amount', $paymentColumns)) {
                $paymentData['amount'] = $field->price_per_hour * 2;
            }
            ...
            if (in_array('paid_at', $paymentColumns)) {
                $paymentData['paid_at'] = Carbon::now()->subHours(rand(1, 24));
            }
