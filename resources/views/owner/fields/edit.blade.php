<x-app-layout>
    ...
    <div class="py-6 max-w-2xl mx-auto">
        ...
        <form method="POST" action="{{ route('owner.fields.update', $field) }}" enctype="multipart/form-data" class="bg-white rounded-xl border border-[#e2e8f0] p-6 shadow-sm">
            @csrf @method('PUT')
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="name" value="{{ old('name', $field->name) }}" required placeholder="Ví dụ: Sân cỏ nhân tạo 7 người A" class="w-full px-3.5 py-2 border border-[#e2e8f0] focus:ring-2 focus:ring-[#0f172a]/10 rounded-lg text-sm focus:outline-none focus:border-[#0f172a] transition-all">
                </div>
                <div>
                    ...
                    <select name="sport_id" required class="w-full px-3.5 py-2 border border-[#e2e8f0] rounded-lg text-sm focus:outline-none focus:border-[#0f172a] focus:ring-2 focus:ring-[#0f172a]/10 transition-all">
                        @foreach($sports as $s)
                            <option value="{{ $s->id }}" {{ (old('sport_id', $field->sport_id) == $s->id) ? 'selected' : '' }}>{{ $s->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    <label class="text-[11px] font-bold text-[#45464d] uppercase tracking-wider block mb-1.5">Mã sân</label>
                    <input type="text" name="code" value="{{ old('code', $field->code) }}" placeholder="Ví dụ: FIELD_A" class="w-full px-3.5 py-2 border border-[#e2e8f0] focus:ring-2 focus:ring-[#0f172a]/10 rounded-lg text-sm focus:outline-none focus:border-[#0f172a] transition-all">
                </div>
                <div>
                    ...
                    <input type="number" name="price_per_hour" value="{{ old('price_per_hour', $field->price_per_hour) }}" required min="0" placeholder="Ví dụ: 150000" class="w-full px-3.5 py-2 border border-[#e2e8f0] focus:ring-2 focus:ring-[#0f172a]/10 rounded-lg text-sm focus:outline-none focus:border-[#0f172a] transition-all">
                </div>
                <div>
                ...
                <div>
                    ...
                    <input type="time" name="open_time" value="{{ old('open_time', $field->open_time?->format('H:i')) }}" class="w-full px-3.5 py-2 border border-[#e2e8f0] focus:ring-2 focus:ring-[#0f172a]/10 rounded-lg text-sm focus:outline-none focus:border-[#0f172a] transition-all">
                </div>
                <div>
                    <label class="text-[11px] font-bold text-[#45464d] uppercase tracking-wider block mb-1.5">Giờ đóng cửa</label>
                    <input type="time" name="close_time" value="{{ old('close_time', $field->close_time?->format('H:i')) }}" class="w-full px-3.5 py-2 border border-[#e2e8f0] focus:ring-2 focus:ring-[#0f172a]/10 rounded-lg text-sm focus:outline-none focus:border-[#0f172a] transition-all">
                </div>
                <div class="md:col-span-2">
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="address" value="{{ old('address', $field->address) }}" placeholder="Số nhà, ngõ, đường, quận/huyện..." class="w-full px-3.5 py-2 border border-[#e2e8f0] focus:ring-2 focus:ring-[#0f172a]/10 rounded-lg text-sm focus:outline-none focus:border-[#0f172a] transition-all">
                </div>
                <div class="md:col-span-2">
                    <label class="text-[11px] font-bold text-[#45464d] uppercase tracking-wider block mb-1.5">Mô tả</label>
                    <textarea name="description" rows="4" placeholder="Nhập mô tả chi tiết về sân (chất lượng cỏ, dịch vụ đi kèm, lưu ý...)" class="w-full px-3.5 py-2 border border-[#e2e8f0] focus:ring-2 focus:ring-[#0f172a]/10 rounded-lg text-sm focus:outline-none focus:border-[#0f172a] transition-all">{{ old('description', $field->description) }}</textarea>
                </div>
            </div>
            <div class="mt-8 flex gap-3 pt-4 border-t border-[#e2e8f0]">
                <button type="submit" class="px-6 py-2.5 bg-[#0f172a] text-[#4ade80] rounded-lg font-bold text-xs uppercase tracking-wider hover:bg-slate-800 transition-colors shadow-sm">Cập nhật</button>
