<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="font-bold font-heading text-2xl text-[#0f172a] uppercase tracking-wide">Sửa Sân</h2>
        ...
        <form method="POST" action="{{ route('owner.fields.update', $field) }}" enctype="multipart/form-data" class="bg-white rounded-xl border border-[#e2e8f0] p-6 shadow-sm">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="name" value="{{ old('name', $field->name) }}" required class="w-full px-3 py-2 border border-[#e2e8f0] rounded-lg text-sm focus:outline-none focus:border-[#0f172a]">
                </div>
                <div>
                ...
                <div>
                    ...
                    <input type="text" name="code" value="{{ old('code', $field->code) }}" class="w-full px-3 py-2 border border-[#e2e8f0] rounded-lg text-sm focus:outline-none focus:border-[#0f172a]">
                </div>
                <div>
                    ...
                    <input type="number" name="price_per_hour" value="{{ old('price_per_hour', $field->price_per_hour) }}" required min="0" class="w-full px-3 py-2 border border-[#e2e8f0] rounded-lg text-sm focus:outline-none focus:border-[#0f172a]">
                </div>
                <div>
                    <label class="text-xs font-semibold text-[#45464d] uppercase tracking-wider block mb-1">Giờ mở cửa</label>
                    <input type="time" name="open_time" value="{{ old('open_time', $field->open_time?->format('H:i')) }}" class="w-full px-3 py-2 border border-[#e2e8f0] rounded-lg text-sm focus:outline-none focus:border-[#0f172a]">
                </div>
                <div>
                    <label class="text-xs font-semibold text-[#45464d] uppercase tracking-wider block mb-1">Giờ đóng cửa</label>
                    <input type="time" name="close_time" value="{{ old('close_time', $field->close_time?->format('H:i')) }}" class="w-full px-3 py-2 border border-[#e2e8f0] rounded-lg text-sm focus:outline-none focus:border-[#0f172a]">
                </div>
                <div>
                ...
                <div class="md:col-span-2">
                    ...
                    @if($field->image_url)
                        <div class="mb-2"><img src="{{ $field->image_url }}" class="h-24 w-auto rounded-lg object-cover"></div>
                    @endif
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="address" value="{{ old('address', $field->address) }}" class="w-full px-3 py-2 border border-[#e2e8f0] rounded-lg text-sm focus:outline-none focus:border-[#0f172a]">
                </div>
                <div class="md:col-span-2">
                    <label class="text-xs font-semibold text-[#45464d] uppercase tracking-wider block mb-1">Mô tả</label>
                    <textarea name="description" rows="3" class="w-full px-3 py-2 border border-[#e2e8f0] rounded-lg text-sm focus:outline-none focus:border-[#0f172a]">{{ old('description', $field->description) }}</textarea>
                </div>
            </div>
            <div class="mt-6 flex gap-3">
                <button type="submit" class="px-6 py-2.5 bg-[#0f172a] text-[#4ade80] rounded-lg font-bold text-xs uppercase tracking-wider hover:bg-slate-800">Cập nhật</button>
