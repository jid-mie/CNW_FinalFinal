...
<div class="p-6 bg-[#f8fafc] min-h-screen flex flex-col items-center justify-start pt-12">
    ...
    <div class="w-full max-w-xl bg-white rounded-3xl shadow-xl border border-slate-100 overflow-hidden animate-in fade-in duration-200">
        ...
        <div class="bg-[#111625] px-6 py-5 text-white flex justify-between items-center">
            <div>
                <h3 class="text-xs font-black uppercase tracking-wider text-[#52ffa2]">Cấu hình hệ thống</h3>
                <h1 class="text-base font-bold text-slate-200 mt-0.5">Chỉnh sửa môn thể thao</h1>
            </div>
        ...
        <form action="{{ route('admin.sports.update', $sport->id) }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-5">
            @csrf
            ...
            <div>
                ...
                <input type="text" name="name" value="{{ old('name', $sport->name) }}" placeholder="Ví dụ: Cầu lông, Pickleball, Bóng rổ..." class="w-full p-3 border border-slate-200 rounded-xl text-xs focus:outline-none focus:border-slate-800 font-semibold text-slate-700 shadow-sm" required>
            </div>
            ...
            <div>
                ...
                <textarea name="description" rows="4" placeholder="Nhập thông tin mô tả tóm tắt về quy định hoặc cơ sở vật chất đặc trưng của bộ môn..." class="w-full p-3 border border-slate-200 rounded-xl text-xs focus:outline-none focus:border-slate-800 text-slate-600 font-medium shadow-sm resize-none">{{ old('description', $sport->description) }}</textarea>
            </div>
